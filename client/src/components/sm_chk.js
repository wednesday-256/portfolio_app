...
const ChBox = (props) => {
  ...
  const single_handler = () => {
    ...
    let btnT = setTimeout(() => {
      let chk = document.getElementById("chbox");
      if (chk === null) {
        clearTimeout(btnT);
        return;
      }
  ...
  const join_game = () => {
    let code = document.querySelectorAll("#ch_code");
    let use;
    if (code[0].value === "") {
      use = 1;
    } else {
  ...
  return (
    <div id="chbox" className="d-flex flex-column justify-content-between">
      <Button
