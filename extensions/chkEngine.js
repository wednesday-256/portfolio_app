...
class CGame{
	...
	king_nextpoints(piece){
		...
		capl = false; capr= false; warp= {1:2, 2:1}
	...
	move_piece(prev, pos){
		...
		if ( piece.clr != this.present_turn ){ return [9, 'Invalid turn!!'] }

		if( piece == undefined ){ return  [9, 'Invalid Piece']}
		...
		piece.row =Number(pos[0]); piece.col = Number(pos[1])
		...
		this.dead_pieces.length > 0 ?  check = this.get_nextpoints(piece) : check = false
	...
	update_expected(){
		...
		pieces.forEach((x)=>{
			...
			} else if ( res.length > 0 ) { norm_p.push(x.get_positions()) }
		})

		if ( cap_p.length > 0 ){this.expected_pieces = cap_p}
		else { this.expected_pieces = norm_p }
	}
	...
	check_winner(){
		...
		if (cnt == 0 || clean_arr(pos).length == 0){ this.winner = 2; this.player2_score += 1 }
		...
		if (cnt == 0 || clean_arr(pos).length == 0){ this.winner = 1 ; this.player1_score += 1}
	}
...
class Piece{
	...
	constructor(clr, row, col, is_king){
		if( is_king ){ this.is_king = is_king }
		this.clr = clr;
		this.row = Number(row);
		this.col = Number(col);
	}
...
class CBoard extends CGame{
	...
	constructor(board){
		...
		board.black_pieces.concat(board.white_pieces).forEach((obj)=>{
			if ( obj.clr == 1 ){
				this.white_pieces.push( new Piece(obj.clr, obj.row, obj.col, obj.is_king))
			} else { 
				this.black_pieces.push( new Piece(obj.clr, obj.row, obj.col, obj.is_king))
			}
		})
	}
...
//resul class
class Result{
	constructor(sc, pc, pos){
		sc ? this.score = sc: null
		pc? this.piece = pc: null
		pos? this.position = pos:null
	}
}
...
class CEngine{
	...
	get_next_move(board, player, depth, alpha, beta){
		...
		let best;
		if (board.first_move){
			return{ 'score': 0, 'piece':[2, [1,3,5,7][Math.floor((Math.random()* 3))]], 'position':[3, [0,2,4,6][Math.floor((Math.random()* 3))]] }
		}

		if ( board.expected_pieces.length <= 0 || depth <= 0 ){return {'score': board.get_player_score(other_player), 'position': null}}
		...
		if ( player == max_player ){best = {'position' : null, 'score': -Infinity}}
		else {best = {'position': null, 'score': Infinity}}
			...
			for ( let move of board.get_nextpoints(piece)){
				...
				try{
					eval_score = this.get_next_move(
						board, other_player, depth -1, alpha,beta 
					)
					
				}
				catch(e){
					eval_score = { 'score': board.get_player_score(other_player),
						position: null }
				}
